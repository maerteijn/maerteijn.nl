...
![Django sync or async](/images/django-sync-or-async.png#left =215x147)In the current world of (cloud-)hosted external services, a requirement of integrating such a service is more the rule than exception. When you integrate such a service in your Django project, you can run into concurrency issues pretty fast, especially if there is a lot of latency. We have an [async, coroutine based stack now in Django](https://docs.djangoproject.com/en/5.1/topics/async/), would it be beneficial? But what if we want to integrate the external service or API into a "sync" codebase or do not want to develop with an async stack?


## Know thy options

So before we can decide if we want to do this "sync or async", we first need a bit of background information and understand the terminology and implications before we can make a considered assessment. "async" is the newest kid on the block, but Django's codebase still has a lot of "sync" parts (like all generic views and forms) and it adds complexity which might not outweigh the benefits.

But what is this async all about? It's all about:
...
We are going to benchmarks a simple Django view which calls an "external" API which returns a random country. This "external" API is slow on purpose (300ms default latency), and we are going to call this external API multiple times to test if the view is doing these API calls concurrently (which means the request should be as slow as the slowest API call as we call the API in parallel using a `ThreadPool`).
